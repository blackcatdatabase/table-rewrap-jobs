...
namespace BlackCat\Database\Packages\RewrapJobs\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\RewrapJobs\Dto\RewrapJobDto;
use BlackCat\Database\Packages\RewrapJobs\Mapper\RewrapJobDtoMapper;
 ...
 */
final class RewrapJobsAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private RewrapJobRepository $rewrapJobRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'rewrap_jobs' => $this->rewrapJobRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\RewrapJobs\ModuleException("Record {$idStr} not found for locking.");
            }
