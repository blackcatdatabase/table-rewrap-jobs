...
namespace BlackCat\Database\Packages\RewrapJobs;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RewrapJobs\Criteria;
use BlackCat\Database\Packages\RewrapJobs\Repository\RewrapJobRepositoryInterface;
use BlackCat\Database\Packages\RewrapJobs\Repository\RewrapJobRepository as GeneratedRepository;
...
{
  private RewrapJobRepositoryInterface $repo;
  ...
  public function __construct(private Database $db)
  {
    ...
    public function _setRepositoryForTests(RepoContract $repo): void
    {
        ...
        if (!$repo instanceof RewrapJobRepositoryInterface) {
            // Allow any repo that conforms to the interface even if not generated
            throw new \InvalidArgumentException('Expected RewrapJobRepositoryInterface');
        }
        $this->repo = $repo;
    }
    ...
    public function getByUnique(array $keys, bool $asDto = false): array|\BlackCat\Database\Packages\RewrapJobs\Dto\RewrapJobDto|null { return $this->repo->getByUnique($keys, $asDto); }
    public function getById(int|string|array $id, bool $asDto = false): array|\BlackCat\Database\Packages\RewrapJobs\Dto\RewrapJobDto|null { return $this->repo->getById($id, $asDto); }
