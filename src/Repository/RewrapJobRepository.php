...
namespace BlackCat\Database\Packages\RewrapJobs\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\RewrapJobs\Definitions;
use BlackCat\Database\Packages\RewrapJobs\Criteria;
use BlackCat\Database\Packages\RewrapJobs\Dto\RewrapJobDto as Dto;
use BlackCat\Database\Packages\RewrapJobs\Mapper\RewrapJobDtoMapper as RowMapper;
...
final class RewrapJobRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\RewrapJobs\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\RewrapJobs\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\RewrapJobs\Dto\RewrapJobDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\RewrapJobs\Dto\RewrapJobDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\RewrapJobs\Mapper\RewrapJobDtoMapper::fromRow($row);
    }
