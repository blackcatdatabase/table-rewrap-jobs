...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 2 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 4 · Changelog: fresh (18 d)</div>
</div>
...
- Pkg folder: [packages\rewrap-jobs](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\rewrap-jobs)
...
graph LR
  ...
  rewrap_jobs["rewrap_jobs"]:::center
  rewrap_jobs -->|FK| key_wrappers["key_wrappers"]:::link
  rewrap_jobs -->|FK| kms_keys["kms_keys"]:::accent
...
- Outbound (depends on): "key_wrappers", "kms_keys"
...
- id (key)
- key_wrapper_id (key)
- target_kms1_key_id (key)
- target_kms2_key_id (key)

## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package rewrap-jobs -Apply
